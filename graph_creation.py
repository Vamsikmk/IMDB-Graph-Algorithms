...
#         unique_nconst = self.df['nconst'].unique()
#         for nconst in unique_nconst:
#             titles = self.df[self.df['nconst']== nconst]
#             titles_col =titles[['nconst','primaryName','primaryProfession']].iloc[0]
#             if(titles_col['primaryProfession']== 'director'):
#                 profession_dict1[nconst] = titles_col['primaryName'] + '_d'
#             else:
#                 profession_dict1[nconst] = titles_col['primaryName'] + '_a'
#         print(len(profession_dict))
#         print(len(profession_dict1))
#         if profession_dict == profession_dict1:
...
class MovieNetwork:
    ...
    def add_edge(self, node1, node2, nconst_ar_dr, weight = 1):        
        movies1 = set(self.name_movie_dict[node1])
        movies2  = set(self.name_movie_dict[node2])
    ...
    def create_graph(self):
        ...
        #example graph looks like:
          # {'nm0962553': {'nm8630849': 3,
             ...
             # 'nm6225202': {}}
        unique_nconst1 = self.nconst_ar_dr.keys()
        for node in unique_nconst1:
            self.add_node(node)
            
        unique_nconst = list(self.nconst_ar_dr.keys())
        #print(unique_nconst)

        for i, n1 in enumerate(unique_nconst):
            for n2 in unique_nconst[i+1:]:
               # print(i,n1,n2)
                self.add_edge(n1, n2, self.nconst_ar_dr)
                
        # for n1 in unique_nconst:
        #     for n2 in unique_nconst:
        #         if n1 != n2 :
        #             self.add_edge(n1, n2,self.nconst_ar_dr)
        print((self.graph))
        return self.graph
